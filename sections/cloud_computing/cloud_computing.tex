...
\section{The different Layers In Cloud Computing}
It is important to get an overview over the Cloud Computing landscape, or as the cloud native foundation expresses it: Cloud Native Landscape.

There is a distinction between the two, but they also draw many parallels.

\begin{figure}[!htb]
  \includegraphics[width=\textwidth]{cloud_computing_cloud_native_landscape}  
  \caption{The cloud native landscape, defined by the Cloud Native Computing Foundation}
...
The infrastructure is a higher level, determining where the application is distributed.

\subsection{Architecture}
When using the word architecture, everything revolving designing the system on both a application and infrastructure level is incorporated.
