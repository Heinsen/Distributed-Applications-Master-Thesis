...
\section{Summary}
There are inherently many problems with sharing a database between several services, and it should therefore be avoided. The main issues with this form of integration is the introduction of a single point of failure, incurring a big risk for depending services. Limiting the maximum amount of available capacity can also hurt availability, with a too low capacity, the database might get overloaded, completely coming to a halt or not responding on a certain percentage of the requests. This architecture makes it harder to introduce resilience, by introducing several limiting factors by nature.